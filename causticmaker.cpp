        ...
        for(int j = 0; j < m_cols; j++){
            ...
            if( i > fmin(y1, y2) && i < fmax(y3, y4)){
                if(j > fmin(x1, x4) && j < fmax(x2, x3)){
                    if( m_caustic[y * m_cols + x][0] > 300 ){
                        result[i * m_cols + j][0] = m_caustic[y * m_cols + x][0];
                        result[i * m_cols + j][1] = m_caustic[y * m_cols + x][1];
                        result[i * m_cols + j][2] = m_caustic[y * m_cols + x][2];
                    } else if( m_caustic[y * m_cols + x][0] > 3 ){
